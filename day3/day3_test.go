...
func movePoint(p point, d rune) point {
	switch d {
	case 'U':
		return point{
			x: p.x,
...
func points(input []move) []wirePoint {
	pt := point{}
	var result []wirePoint
	steps := 1
	for _, mv := range input {
		for i := 0; i < mv.distance; i++ {
			pt = movePoint(pt, mv.direction)
			result = append(result, wirePoint{
				point: pt,
				steps: steps,
			})
			steps++
...
func pointLess(a, b point) bool {
	//aDist := distance(point{}, a)
	//bDist := distance(point{}, b)
	//if aDist < bDist {
	//	return true
	//}
	if a.x < b.x {
...
//	b := points(moves("U62,R66,U55,R34,D71,R55,D58,R83"))
//	ptA := point{x: 155, y: 5}
//	sort.Slice(b, func(i, j int) bool {
//		return pointLess(b[i].point, b[j].point)
//	})
//
//	fmt.Println(lessOrEq(ptA, b[0]))
//	fmt.Println(lessOrEq(ptA, b[359]))
//	fmt.Println(lessOrEq(ptA, b[360]))
//	for _, p := range b[359:] {
//		assert.True(t, lessOrEq(ptA, p))
//	}
//	for i, p := range b[:359] {
//		assert.False(t, lessOrEq(ptA, p), fmt.Sprintf("%d - %d,%d",i, p.x, p.y))
//	}
//	fmt.Println(ptA)
//
////359
...
func (n intersection) latency() int {
	return n.ptA.steps + n.ptB.steps
}

func intersects(a, b []wirePoint) []intersection {
	sort.Slice(a, func(i, j int) bool {
		return pointLess(a[i].point, a[j].point)
	})
	sort.Slice(b, func(i, j int) bool {
		return pointLess(b[i].point, b[j].point)
	})
	...
	for ia, ptA := range a[204:] {
		_ = ia
		j := sort.Search(len(b), func(i int) bool {
			if eqPoint(ptA.point, b[i].point) {
				return true
			}
			return pointLess(ptA.point, b[i].point)
		})
		if j < len(b) && eqPoint(b[j].point, ptA.point) {
			result = append(result, intersection{ptA: ptA, ptB: b[j]})
		}
		//for ib, ptB := range b {
		//	if eqPoint(ptA, ptB) {
		//		fmt.Println(ia)
